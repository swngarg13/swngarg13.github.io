...
function start() {
  ...
  if (navigator.mediaDevices.getUserMedia) {
    navigator.mediaDevices.getUserMedia(constraints)
      ...
      .then(() => {
        serverConnection = new WebSocket('ws://' + window.location.hostname + ':8000/ws');
...
function gotMessageFromServer(message) {
  ...
  if (signal.displayName && signal.dest == 'all') {
    ...
    serverConnection.send(JSON.stringify({ roomId: 'skg', 'displayName': localDisplayName, 'uuid': localUuid, 'dest': peerUuid }));
...
function gotIceCandidate(event, peerUuid) {
  if (event.candidate != null) {
    serverConnection.send(JSON.stringify({ roomId: 'skg', 'ice': event.candidate, 'uuid': localUuid, 'dest': peerUuid }));
  }
}
...
function createdDescription(description, peerUuid) {
  ...
  peerConnections[peerUuid].pc.setLocalDescription(description).then(function () {
    serverConnection.send(JSON.stringify({ roomId: 'skg', 'sdp': peerConnections[peerUuid].pc.localDescription, 'uuid': localUuid, 'dest': peerUuid }));
  }).catch(errorHandler);
}
